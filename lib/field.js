...
extend(PField.prototype, {
  template: heredoc(function(oneline) {/*
    <div id="{{ root.id }}" class="field text-field {{#if !root.label}}no-label{{/if}}" data-type="{{ root.type }}" data-property="{{ root.property }}">
      <p class="field-label">{{ root.label }}</p>
      <input class="field-block" type="text" value="{{ root.value }}" style="{{#if root.label}}margin-top: 10px;{{/if}}">
      <p class="error"></p>
    </div>
  */}),

  render: function() {
    return Crox.render(this.template, extend({}, this.opts, { property: this.property, id: this.id.slice(1) }))
  },
