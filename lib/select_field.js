...
function SelectField(p, obj) {
  this.id = '#j-editor-select-field-' + n++
...
extend(SelectField.prototype, {
  template: heredoc(function(oneline) {/*
    <div id="{{ root.id }}" class="field select-field" data-type="{{ root.type }}" data-property="{{ root.property }}">
      ...
      <div class="field-input-box">
        <div class="field-input">{{ root.options[root.value] }}</div>
  ...
  */}),
  ...
  bind: function() {
    var $e = $(this.id)
    $e.find('.field-input-box').on('click', this.triggerDropdown.bind(this))
  ...
  sync: function() {
    ...
    var input = $e.find('.field-input-box')
