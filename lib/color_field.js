...
function ColorField(p, opts) {
  ...
  this.property = p
  this.opts = extend({ palette: _palette, label: '颜色' }, opts)
}

inherits(ColorField, Field)

extend(ColorField.prototype, {
  template: heredoc(function(online) {/*
    <div id="{{ root.id }}" class="field color-field" data-type="{{ root.type }}" data-property="{{ root.property }}">
