...
_.extend(ColorField.prototype, {
  ...
  */}),
  ...
  bind: function() {
    ...
    if(palette) {
      this.renderColorGroup(palette)
    }
    else if(source) {
      var image_field = el.parent().find('[data-property="' + source + '"]')
      var _input = image_field.find('input').first()

      image_field.on('color:imageloaded', function(e) {
        self.change(_input.val())
      })
    }
    else {
      this.renderColorGroup(_palette)
    }

    el.on('click', this.select.bind(this))

  },

  renderColorGroup: function(colors) {
